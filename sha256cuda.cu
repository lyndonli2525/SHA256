...
Objective: SHA-256 implementation in C according to the documentation set out by
NIST References:
...
// JOB *JOB_init( /*BYTE * data,*/ size_t len) {
//	JOB * j;
//	//size_t data_len = strlen((char *)data);
//    cudaMallocManaged(&j, sizeof(JOB));
//	//cudaMallocManaged(&(j->data), len);
//	j->size = len;
//	//j->data = data;
//	for (int i = 0; i < 64; i++) {
//	  j->digest[i] = 0x00;
...
__global__ void sha256_cuda(JOB **jobs, int n) {
  ...
  if (idx < n) {
    SHA256_CTX ctx;
    sha256_init(&ctx);
    sha256_update(&ctx, jobs[idx]->data, jobs[idx]->size);
    sha256_final(&ctx, jobs[idx]->digest);
  }
}
...
void runJobs(JOB **jobs, int num_jobs) {
  ...
  sha256_cuda<<<numBlocks, blockSize>>>(jobs, num_jobs);
}
